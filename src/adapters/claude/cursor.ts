...
/**
 * Idempotent per-transcript file cursor (issue #5 review note).
 *
 ...
 *   - `lastUuid`      — last committed record uuid, for cross-checking
...
export const DEFAULT_CURSOR_DIR = 'data/cursors/claude';
...
export function newCursor(path: string): TranscriptCursor {
  return { path, fileId: null, byteOffset: 0, lastUuid: null, parserVersion: PARSER_VERSION };
}
...
export async function loadCursor(dir: string, transcriptPath: string): Promise<CursorLoad> {
  try {
    ...
    if (parsed.parserVersion !== PARSER_VERSION) return { status: 'invalidated' };
