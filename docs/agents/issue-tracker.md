...
When this directory is inside a GitHub clone, infer the repo from `git remote -v`; `gh` does this automatically when run inside a clone. If no Git remote is configured yet, pass `--repo OWNER/REPO` once the GitHub repo is known.
